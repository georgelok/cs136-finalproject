...
for bid in num_bids :
	for good in num_goods :
		...
		for i in range(5) :
			test = parse_arbitrary('testfiles/arbitrary_nodomchk/arbitrary-'+good+'-'+bid+'-000'+str(i)+'.txt')
			start = time.time()
...
for bid in num_bids :
	...
	for good in num_goods :
		print "testing 3-atom-"+good+"-"+bid
		for i in range(5) :
			c = cplex.Cplex('testfiles/arbitrary_nodomchk/arbitrary-'+good+'-'+bid+'-000'+str(i)+'.lp')
			# Parameters
			# Aggressive Probing
			# 5% away from optimal
