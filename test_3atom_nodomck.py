...
for bid in num_bids :
	for good in num_goods :
		...
		for i in range(5) :
			test = parse_3atom('testfiles/3atom_nodomck/3atom-'+good+'-'+bid+'-000'+str(i)+'.txt')
			start = time.time()
...
for bid in num_bids :
	...
	for good in num_goods :
		print "testing 3-atom-"+good+"-"+bid
		for i in range(5) :
			c = cplex.Cplex('testfiles/3atom_nodomck/3atom-'+good+'-'+bid+'-000'+str(i)+'.lp')
			# Parameters
			# Aggressive Probing
			# 5% away from optimal
...
with open('resultfiles/3atom-greedy-nodomck.csv', 'wb') as f :
	wr = csv.writer(f)
	wr.writerows(greedy_results)

with open('resultfiles/3atom-ibm-nodomck.csv', 'wb') as f :
	wr = csv.writer(f)
